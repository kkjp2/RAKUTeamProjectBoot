...
public class ReactionService {
    ...
    @Transactional
    public void reactToReview(Long reviewId, String userKey, String reactionType) {
        // 리뷰 가져오기
    ...
    private void updateReactionCount(MoveReviewEntity review, ReactionType type, int delta) {
        ...
        if (type == ReactionType.LIKE) {
            countEntity.setLikeCount(countEntity.getLikeCount() + delta);
        } else if (type == ReactionType.DISLIKE) {
            countEntity.setDislikeCount(countEntity.getDislikeCount() + delta);
        }
