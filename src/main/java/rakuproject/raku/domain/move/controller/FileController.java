...
public class FileController {
    ...
    public ResponseEntity<String> getFileByUuid(@PathVariable String uuid) {
        UploadFileEntity fileEntity = uploadFileRepository.findByUuid(uuid)
                .orElseThrow(() -> new RuntimeException("파일을 찾을 수 없습니다"));

        // 拼接路径，包含folderPath与uuid_fileName
        String fileUrl = "http://localhost:8080/uploads/"
                + fileEntity.getFolderPath() + "/"
                + fileEntity.getUuid() + "_" + fileEntity.getFileName();
