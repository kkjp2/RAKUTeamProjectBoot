package rakuproject.raku.domain.email.service;
...
import rakuproject.raku.global.redis.RedisUtil;
...
public class MailSendService {
    ...
    public boolean CheckAuthNum(String email,String authNum){
        ...
        if (email == null || email.isEmpty() || authNum == null || authNum.isEmpty()) {
            logger.info("Invalid input: email or authNum is null or empty.");
            return false;
        }

        String storedEmail = redisUtil.getData(authNum);
        if (storedEmail == null) {
            logger.info("Authentication failed: authNum not found in Redis.");
            return false;
        }
        ...
        if (storedEmail.equals(email)) {
            ...
            logger.info("Authentication failed: authNum does not match email.");
            return false;
        }
    }
    ...
    public EmailResponseDto joinEmail(String email) {
        ...
        return EmailResponseDto.builder()
                .code(Integer.toString(authNumber)) // 인증번호를 code 필드에 저장
                .build();
    }
